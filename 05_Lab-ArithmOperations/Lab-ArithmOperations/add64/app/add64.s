...
endless
		...
		; R0 value lower 32 bits
		; R1 higher 32 bits
		; R2
		; R3
		; R7 temp
		LDR R7,=ADDR_DIP_SWITCH_31_0			; Load the address of switch DS31-0 into R7 
		LDR R2,[R7]							; Load the value from DS31-0 into R2

		LDR R3, =0								; Load the value 0 into R3

		ADDS R0, R0, R2							; Add the values of R0 and R2 and save it to R0
		ADCS R1, R1, R3							; Add the values of R1 and R2 with carry! and save it to R1
		
		
		LDR R7,=ADDR_LCD_BIN					; Load the address of LCD Display
		STR R0,[R7]							; Store the value in R7
		STR R1,[R7,#8]						; Store the value of R1 into R7, but shiftet 8 bits to the left
